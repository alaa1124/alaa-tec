...
class Finishurantee(models.Model):
    ...
    def compute_amount(self):
        for rec in self:
            ...
            for ras in raise_guarantee:
                ...
                rec.raise_amount = r
            down_amount = self.env['guarantee.reduction'].search(
                [('letter_guarantee_id', '=', rec.letter_guarantee_id.id)])
            x = 0.0
            dow_cov = 0.0
        ...
        for rec in self:
            if rec.letter_guarantee_id:
                downs = self.env['guarantee.closing'].search(
                    [('id', '!=', rec.id), ('letter_guarantee_id', '=', rec.letter_guarantee_id.id)])
                if downs:
                    raise UserError(_('This Gurantee Letter Closed Before'))
    ...
    def cancel_button(self):
        for rec in self:
            ...
            if rec.move_id:
                ...
                ex = self.env['guarantee.extension'].search([('letter_guarantee_id', '=', rec.letter_guarantee_id.id)])
                for dd in dw:
                    dd.is_close = False
                for rais in rs:
                    rais.is_close = False
    ...
    def unlink(self):
        for rec in self:
            if rec.state != 'draft':
                raise ValidationError("You can delete only draft")
        res = super(Finishurantee, self).unlink()
        return res
