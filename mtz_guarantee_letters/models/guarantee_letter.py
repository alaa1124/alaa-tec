...
class AccountMove(models.Model):
    ...
    gl_id = fields.Many2one("guarantee.letter")
...
class LetterOfGurantee(models.Model):
    ...
    def confirm_button(self):
        for rec in self:
            ...
            move = self.env['account.move'].create({
                'journal_id': rec.journal_id.id,'date': rec.start_date,"gl_id":self.id})
            ...
            self.env['account.move.line'].with_context(check_move_validity=False).create(
                {
                    'move_id': move.id,'account_id': account_id,'name': 'Gurantee Letter','credit': rec.cover_amount,})
            move.action_post()
            rec.move_id = move.id
            if rec.is_expenses :
                config = self.env['guarantee.letter.setting'].search([('id', '=', rec.config_id.id)])
                ...
                self.env['account.move.line'].with_context(check_move_validity=False).create(
                    {
                        'move_id': move.id,'account_id': account_id,'name': 'Gurantee Letter','analytic_account_id': rec.analytic_id.id,'credit': rec.expenses_amount,})
                move.action_post()
    ...
    def unlink(self):
        ...
        res = super(LetterOfGurantee, self).unlink()
        return res
