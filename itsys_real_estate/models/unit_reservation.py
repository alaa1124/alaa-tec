...
class unit_reservation(models.Model):
    ...
    def auto_cancel_reservation(self):
        try:
            ...
            reservation_hours = self.env['res.config.settings'].browse(
                self.env['res.config.settings'].search([])[-1].id).reservation_hours if self.env[
    ...
    def _prepare_lines(self,first_date):
        ...
        if self.template_id:
            ...
            mon = self.template_id.duration_month or 1
...
class loan_line_rs(models.Model):
    ...
    loan_id= fields.Many2one('unit.reservation', 'Installment',ondelete='cascade', readonly=True)
