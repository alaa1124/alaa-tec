...
class rental_contract(models.Model):
    ...
    # Building Unit Info
    building_unit = fields.Many2one('product.template', 'Building Unit', required=True,
    ...
    account_security_deposit = fields.Many2one('account.account', 'Security Deposit Account',
                                               default=lambda self: self.env['res.config.settings'].browse(
                                                   self.env['res.config.settings'].search([])[
                                                       -1].id).security_deposit_account.id if self.env[
    ...
    def prepare_lines(self):
        ...
        for rec in self:
            ...
            if rec.insurance_fee > 0:
                rental_lines.append(
                    (0, 0, {'serial': i, 'amount': rec.insurance_fee, 'date': rec.date, 'name': _('Insurance Fee')}))
                i += 1
            ...
            while new_date < date_to:
                ...
                if new_date > date_to:  # and date_from.month!=date_to.month:
                    ...
                    if first_line:
                        rental_lines.append(
                            (0, 0, {'serial': i, 'amount': rental_fee, 'date': loan_date, 'name': _('Rental Fee')}))
...
class loan_line_rs_rent(models.Model):
    ...
    contract_building_unit = fields.Many2one(related='loan_id.building_unit', string="Building Unit",
                                             domain=[('is_property', '=', True)], store=True)
