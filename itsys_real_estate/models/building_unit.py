...
class building_unit(models.Model):
    ...
    city_id = fields.Many2one('cities', string='City', related='building_id.region_id.city_id', store=True,
                              readonly=True)
    country_id = fields.Many2one('countries', string='Country', related='building_id.region_id.city_id.country_id',
    ...
    def set_website_ribbon(self):
        self.website_ribbon_id = self.website_ribbon_id.get_by_state(self.state)

    reservation_date = fields.Datetime()
    reservation_end_date = fields.Datetime()
    reserved = fields.Boolean()

    @api.model
    def _cron_set_product_to_free(self):
        """
        Cron method to set product state to 'free' if reservation end date
        is in the past and the product is not currently reserved.
        """
        today = datetime.now()
        units_to_free = self.search([
            ('reserved', '=', False),
            ('reservation_end_date', '!=', False),
            ('reservation_end_date', '<', today),
