...
class Lines(models.Model):
    ...
    previous_quanity = fields.Float(compute='get_previous_amount')
    total_quanity = fields.Float(compute='get_total_quanity')
    display_type = fields.Selection([
        ('line_section', "Section"),
        ('line_note', "Note")], default=False)
    state = fields.Selection(related='eng_id.state')
    stage_prec = fields.Float(string="Precentage Stage")
    precentage = fields.Float()
    ...
    def get_amount(self):
        for rec in self:
            rec.amount = (rec.stage_prec * round(rec.price_unit, 2) * rec.total_quanity) / 100
